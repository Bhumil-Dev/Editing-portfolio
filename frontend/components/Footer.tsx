...
export default function Footer() {
  return (
    <footer className="relative py-12 border-t border-white/5 bg-dark">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full animated-border p-px">
          ...
          <div className="flex items-center gap-6">
            {['Home', 'About', 'Work', 'Contact'].map(l => (
              <a key={l} href={`#${l.toLowerCase()}`}
                className="text-white/20 hover:text-cyan font-mono text-xs tracking-wider transition-colors">
                {l}
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            {socials.map(s => (
              <motion.a
                ...
                className="w-9 h-9 glass rounded-full flex items-center justify-center text-white/30 transition-all duration-300 group"
                style={{ '--hover-color': s.color } as React.CSSProperties}
                onMouseEnter={e => {
                  const el = e.currentTarget
                  el.style.color = s.color
                  el.style.borderColor = `${s.color}40`
                  el.style.boxShadow = `0 0 16px ${s.color}30`
                }}
                onMouseLeave={e => {
                  const el = e.currentTarget
                  el.style.color = ''
        ...
        <div className="mt-8 pt-6 border-t border-white/5 text-center">
          <p className="text-white/15 font-mono text-xs tracking-widest">
            © {new Date().getFullYear()} BHUMIL PRAJAPATI — BUILT WITH THREE.JS + GSAP + NEXT.JS
          </p>
        </div>
      </div>
    </footer>
