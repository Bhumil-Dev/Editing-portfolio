...
export default function Testimonials() {
  ...
  useEffect(() => {
    // Testimonials are stored in Admin model — fetch from public profile or skip
    // For now show empty state until admin adds them via a future testimonials manager
  }, [])
                ...
                <div className="glass p-8 rounded-2xl h-full hover:glass-cyan transition-all duration-300 group border border-white/5 hover:border-cyan/20">
                  ...
                  <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                    <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-cyan/20 flex-shrink-0">
                      <img src={t.photo?.startsWith('/uploads') ? `${API}${t.photo}` : t.photo} alt={t.name} className="w-full h-full object-cover" />
                    </div>
                    <div>
